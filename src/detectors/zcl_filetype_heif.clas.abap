CLASS zcl_filetype_heif DEFINITION PUBLIC FINAL CREATE PUBLIC.
  PUBLIC SECTION.
    INTERFACES zif_filetype_detector.
ENDCLASS.


CLASS zcl_filetype_heif IMPLEMENTATION.
  METHOD zif_filetype_detector~detect.
    CONSTANTS lc_ftyp TYPE x LENGTH 4 VALUE '66747970'.
    CONSTANTS lc_heic TYPE x LENGTH 4 VALUE '68656963'.
    ...
    rv_detected = xsdbool(
         iv_buffer+8(4) = lc_heic
      OR (     ( iv_buffer+8(4) = lc_mif1 OR iv_buffer+8(4) = lc_msf1 )
           AND lv_len >= 16 AND iv_buffer+12(4) = lc_heic ) ).
  ENDMETHOD.
ENDCLASS.
